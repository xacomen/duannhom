...
public class HDCTRepository {
    ...
    public void insert(HDCT model) {
        ...
        DBContext.executeUpdate(sql, model.getMaCTHD(), model.getMaHD_FK(), model.getMaMon_FK(), model.getSoLuong(), model.getDonGia());
    }

    public void update(HDCT model) {
        String sql = "UPDATE CTHOADON SET MaHD_FK=?, MaMon_FK=?,SoLuong=?,DonGia=? where MaCTHD=?";
        DBContext.executeUpdate(sql, model.getMaHD_FK(), model.getMaMon_FK(), model.getSoLuong(), model.getDonGia(), model.getMaCTHD());

    }

    public void delete(String hdct) {
        String sql = "DELETE FROM CTHOADON WHERE MaCTHD=?";
        DBContext.executeUpdate(sql, hdct);
    }
